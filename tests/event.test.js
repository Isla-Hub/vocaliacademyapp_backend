  ...
  it("should create a new event", async () => {
    await supertest(app)
      ...
      .then(async (response) => {
        ...
        expect(dbEvent.createdBy.toString()).toBe(
          newEvent.createdBy.toString()
        );
  ...
  it("should update a event", async () => {
    await supertest(app)
      ...
      .then(async (response) => {
        ...
        expect(response.body.externalAtendants[0].lastName).toBe(
          "Lolita LastName"
        );
        expect(response.body.externalAtendants[0].email).toBe("lolita@me.com");
        expect(response.body.externalAtendants[0].phoneNumber).toBe(
          "123123123"
        );
      });
  });
